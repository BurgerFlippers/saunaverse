...
import { formatDuration, timeAgo } from "./ui/utils";
...
export function PostCard({ post }: PostCardProps) {
  ...
  const saunaDuration = post.saunaSession?.durationMs
    ? formatDuration(post.saunaSession.durationMs)
    : "N/A";
  ...
  return (
    <Card className="heat-wave overflow-hidden rounded-2xl border-[#2C2B36] bg-[#1F1F23]">
          ...
          <p className="font-semibold">
            ...
            {post.achievement.name === "First Sauna Session"
              ...
              : post.achievement.name === "Hottest Sauna"
                ? `reached a new personal best of ${post.saunaSession?.maxTemperature?.toFixed(
                    0,
                  )}°C!`
      ...
      {post.saunaSession && (
        <div className="mt-0 pt-0 pr-3 pb-0 pl-5">
          ...
          <div className="mb-0.5 flex items-center gap-1 text-gray-400">
            ...
            <span className="font-normal" style={{ fontSize: "11px" }}>
              {post.saunaSession.startTimestamp.toLocaleString()}
            </span>
          </div>
          {post.description && (
            <p
              ...
              <>
                <div>
                  ...
                  <p
                    ...
                    style={{ fontSize: "19px" }}
                  >
                    {post.saunaSession.avgTemperature?.toFixed(0)}°
                  </p>
                </div>
                <div>
                  <p
                  ...
                  <p
                    ...
                    style={{ fontSize: "19px" }}
                  >
                    {post.saunaSession.maxTemperature?.toFixed(0)}°
                  </p>
                </div>
                <div>
                  <p
                  ...
                  <p
                    ...
                    style={{ fontSize: "19px" }}
                  >
                    {post.saunaSession.avgHumidity?.toFixed(0)}%
                  </p>
                </div>
              </>
            )}
      ...
      {post.saunaSession && (
        <div className="scrollbar-hide mt-0 overflow-x-auto">
          <div className="ml-5 flex gap-3 pb-0">
            ...
            {!post.saunaSession.manual && (
              <div
                className={`flex-shrink-0 ${post.images.length > 0 ? "w-[85%]" : "w-full"}`}
              >
                {post.saunaSession?.measurements &&
                post.saunaSession.measurements.length > 0 ? (
                  <MemoizedSessionChart
                    measurements={post.saunaSession.measurements}
                  />
                ) : null}
              </div>
            )}
      ...
      <div
        className={
          "border-t border-[#2C2B36] px-5 pt-4" + (!showComments ? " pb-4" : "")
        }
      >
          ...
          <button
            onClick={async () => {
              const shareData = {
                ...
                text: `Check out ${post.createdBy.name}'s sauna session ${post.description || ""}!`,
