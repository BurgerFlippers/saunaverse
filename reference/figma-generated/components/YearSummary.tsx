...
export function YearSummary({ isOpen, onClose }: YearSummaryProps) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div 
          className="fixed inset-0 z-50 bg-[#1F1F23]"
          initial={{ y: '100%' }}
          animate={{ y: 0 }}
          exit={{ y: '100%' }}
        ...
        >
          ...
          <div className="absolute bottom-10 left-0 right-0 flex items-center justify-center gap-8">
            ...
            {currentPage === totalPages ? (
              <button
                onClick={handleClose}
                className="px-5 py-2.5 rounded-full bg-[#D01400] text-white hover:bg-[#b01100] transition-all"
              >
                Close Summary
              </button>
            ) : (
              <button
                onClick={nextPage}
                className="p-3 rounded-full text-white hover:bg-[#2C2B36] transition-all"
              >
